...
// Sizeof, Alignof, Offsetof are compile-time intrinsics intercepted in
// goparser.evalConstExpr (const-context) and comp.compileBuiltin (runtime,
// Sizeof/Alignof only) by matching the qualified symbol name; these stubs
// only exist so that `unsafe.Sizeof` / `unsafe.Alignof` / `unsafe.Offsetof`
// resolve as package members during symbol lookup. They must never run.
