...
func TestSourcesResolveMulti(t *testing.T) {
	...
	tests := []struct {
		...
		{3, "m:first", 1, 4}, // '\n'
		{4, "m:first", 2, 1}, // 'd'
		{7, "m:first", 2, 4}, // one past 'f' (end of source)

		// Second source: "ghi\njkl"
		{8, "m:second", 1, 1},  // 'g'
		{11, "m:second", 1, 4}, // '\n'
		{12, "m:second", 2, 1}, // 'j'
		{15, "m:second", 2, 4}, // one past 'l'
	}
...
func TestSourcesResolveOutOfRange(t *testing.T) {
	var ss Sources
	ss.Add("m:first", "abc")  // base=0, len=3
	ss.Add("m:second", "def") // base=4, len=3
...
func TestSourcesResolveGap(t *testing.T) {
	var ss Sources
	ss.Add("m:first", "ab")   // base=0, len=2
	ss.Add("m:second", "cde") // base=3, len=3
	...
	if name != "m:first" {
		t.Errorf("Resolve(2) name = %q, want %q", name, "m:first")
	}
}

func TestFormatPosMulti(t *testing.T) {
	var ss Sources
	ss.Add("m:repl", "x := 1")       // base=0, len=6
	ss.Add("f:main.go", "func main") // base=7, len=9

	tests := []struct {
		pos  int
		want string
	}{
		{0, "1:1"},          // inline: line:col only
		{5, "1:6"},          // inline: col 6
